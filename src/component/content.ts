  ...
  handler: async (ctx, args) => {
    ...
    if (!existing) {
      const inserted = await ctx.db.insert("settings", {
        ...
        rssEnabled: args.patch?.rssEnabled,
        agentSkillsEnabled: args.patch?.agentSkillsEnabled,
        readinessEndpointEnabled: args.patch?.readinessEndpointEnabled,
        widgetVisible: args.patch?.widgetVisible,
        widgetShowScoreTab: args.patch?.widgetShowScoreTab,
        widgetDesktopCollapse: args.patch?.widgetDesktopCollapse,
        widgetCleanMode: args.patch?.widgetCleanMode,
        widgetShowHumanTab: args.patch?.widgetShowHumanTab,
        widgetShowMachineTab: args.patch?.widgetShowMachineTab,
        widgetShowChatLinks: args.patch?.widgetShowChatLinks,
        widgetShowChatGPT: args.patch?.widgetShowChatGPT,
        widgetShowClaude: args.patch?.widgetShowClaude,
        widgetShowPerplexity: args.patch?.widgetShowPerplexity,
      });
...
export const getCacheStatus = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      widgetDesktopCollapse: settings?.widgetDesktopCollapse ?? true,
      widgetCleanMode: settings?.widgetCleanMode ?? false,
      widgetShowHumanTab: settings?.widgetShowHumanTab ?? true,
      widgetShowMachineTab: settings?.widgetShowMachineTab ?? true,
      widgetShowChatLinks: settings?.widgetShowChatLinks ?? true,
      widgetShowChatGPT: settings?.widgetShowChatGPT ?? true,
      widgetShowClaude: settings?.widgetShowClaude ?? true,
      widgetShowPerplexity: settings?.widgetShowPerplexity ?? true,
